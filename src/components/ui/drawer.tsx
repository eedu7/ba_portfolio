...
const DrawerContent = React.forwardRef<React.ElementRef<typeof DrawerPrimitive.Content>, React.ComponentPropsWithoutRef<typeof DrawerPrimitive.Content>>(({
    ...
    return (<DrawerPortal>
            ...
            <DrawerPrimitive.Content
                ref={ref}
                className={cn("fixed z-50 flex h-auto flex-col border bg-background", (!direction || direction === "bottom") && "inset-x-0 bottom-0 mt-24 rounded-t-[10px]", direction === "right" && "top-0 right-0 w-screen max-w-80 h-full rounded-tl-[10px]", direction === "top" && "top-0 w-screen h-80 rounded-b-[10px]", className)}
                {...props}
            >
